...
const ReportsCard = connect(
  ...
  ({ projectsByRoute: project, batchPlotConfigurationsItems: plots }) => (
    <Card className='mt-4'>
      ...
      <Card.Body hasPaddingVertical={false} className='pt-2'>
        {plots.length ? (
          ...
          <p>
            Create{' '}
            <a href={`/${project.slug}#batch-plotting`}>
              new batch plot configurations
            </a>{' '}
            to view recent configurations here.
          </p>
        )}
      </Card.Body>
    </Card>
