...
export default connect(
  ...
  ({
    ...
    const options = uploadParsers.map((o) => ({
      value: o.name,
      label: o.name,
    }));
    ...
    useEffect(() => {
      if (uploadSelectedParser) {
        const fieldMap = Object.keys(uploadSelectedParser.model).reduce(
          (accum, current) => {
        ...
        <div className='col-9'>
          <Select
            defaultValue={
              uploadSelectedParser
                ? {
                    name: uploadSelectedParser.name,
                    label: uploadSelectedParser.name,
                  }
                : null
            }
